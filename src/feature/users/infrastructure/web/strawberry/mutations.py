...
def convert_result_to_user_type(result) -> UserType:
    ...
    return UserType(
        ...
        status=status_map.get(result.status, UserStatusEnumStrawberry.PENDING_VERIFICATION),
...
class UserMutations:
    ...
    def create_user(self, input: CreateUserInput) -> CreateUserResponse:
        ...
        try:
            ...
            return create_success_response(CreateUserResponse, "User created successfully", user_data)
        except Exception as e:
            return create_error_response(CreateUserResponse, e)

    @strawberry.mutation
    def delete_user(self, user_id: str) -> CreateUserResponse:  # Usando mismo response type
        """Delete a user"""
