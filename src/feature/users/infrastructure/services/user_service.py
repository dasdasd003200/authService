...
class UserService:
    ...
    # ===== QUERIES (Criteria-only approach) =====

    async def find(self, input: UserFindInput) -> UserFindResponse:
        """
        Find users with criteria-only approach.

        Examples of valid criteria:
        - All users: {} (will use defaults)
        - Active users: {"filters": [{"field": "status", "operator": "eq", "value": "active"}]}
        - Search by name: {"filters": [{"operator": "or", "nested_filters": [...]}]}
        """
        try:
    ...
    async def find_one(self, input: UserFindOneInput) -> UserFindOneResponse:
        """
        Find single user with criteria-only approach.

        Examples of valid criteria:
        - By ID: {"filters": [{"field": "id", "operator": "eq", "value": "uuid-here"}]}
        - By email: {"filters": [{"field": "email", "operator": "eq", "value": "user@example.com"}]}
        """
        try:
